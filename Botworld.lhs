...
In the real world, the execution of a computer program is unaffected by the environment \emph{most} of the time (except for its normal input channels): While the contents of a computer's RAM \emph{can} be changed by heating it up with a desk lamp~\cite{desklamp}, they are \emph{usually} not, and an Artificial General Intelligence (AGI) will presumably make use of this fact. Thus, its task becomes to ensure that its Cartesian boundary will \emph{in fact} not be violated in this way, and Botworld attempts to model this problem in a simple way by requiring robots to contend with the possibility that they may be destroyed by other robots.

More problematically, in the real world, the internals of a computer program will always \emph{affect} the environment ---~for example, through the amount of waste heat emitted by the computer~--- but these effects are probably almost always unpredictable enough that an AGI will not be able to improve its performance by trying to carefully choose, e.g., the amount of waste heat it emits. However, an AGI will need to ensure that these unavoidable violations of its Cartesian boundary will \emph{in fact} not make an expected difference to its goals. Botworld side-steps this issue and only requires robots to deal with a much more tractable issue: Dealing with the possibility that one's source code might be read by another agent.

Our model is not realistic, but it is simple to reason about. For all that the robot machines are not reducible, the robots are still embedded in their environment, and they can still be read or destroyed by other agents. We hope that this captures some of the complexity of naturalistic agents, and that it will serve as a useful test bed for formalisms designed to deal with the complexity of naturalistic agents: Although being able to deal with the challenges of Botworld is, perhaps, not a good indicator that a formalism will be able to deal with \emph{all} of the challenges of naturalistic agents, it allows us to see in concrete terms how it deals with at least \emph{some} of them.

In creating Botworld we tried to build something that implementable by a lower-level system, such as Conway's \emph{Game of Life}~\cite{life}. It is useful to imagine such an implementation when considering Botworld games.

Future versions of Botworld may treat the robot bodies as less fundamental objects. In the meantime, we hope that it is possible to picture an implementation where the Cartesian boundary is much less fundamental, and to use Botworld to gain useful insights about agents embedded within their environment. Our intent is that when we apply a formalism for naturalistic agents to the current implementation of Botworld, then there will be a straight-forward translation to an application of the same formalism to an implementation of Botworld in (say) the Game of Life.
